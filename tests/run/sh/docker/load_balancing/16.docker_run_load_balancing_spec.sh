...
$(${docker_compose_cli} -p ldloadbalancing-enabled ps --filter "status=running" --services)
$(${docker_compose_cli} -p ldloadbalancing-enabled ps --filter "status=restarting" --services)
EOF
