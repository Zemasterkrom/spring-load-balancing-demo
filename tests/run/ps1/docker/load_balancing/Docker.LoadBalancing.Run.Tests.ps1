    ...
    Context 'Concrete run behavior check' -Tag ConcreteRun {
        BeforeEach {
            function Assert-ContainersAreStopped {
                $RunningServices = (cmd /c "$([Runner]::Tasks[0].DockerComposeCli.SystemStackComponents[0].Command[0]) $([Runner]::Tasks[0].TaskStartInfo.DockerComposeProcessArgumentList) -p ldloadbalancing-enabled ps --filter status=running --services").Split([Environment]::NewLine)
                $RunningServices += (cmd /c "$([Runner]::Tasks[0].DockerComposeCli.SystemStackComponents[0].Command[0]) $([Runner]::Tasks[0].TaskStartInfo.DockerComposeProcessArgumentList) -p ldloadbalancing-enabled ps --filter status=restarting --services").Split([Environment]::NewLine)
                ...
                foreach ($RunningService in $RunningServices) {
                    switch -Regex ($RunningService) {
                        "^(lddatabase|ldconfig|lddiscovery|ldloadbalancer|ldfront|ldservice|ldservice-two)$" {
