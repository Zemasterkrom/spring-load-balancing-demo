...
'@
        ...
        Context 'Run mode enabled' {
            ...
            It 'checks that the run stage is triggered' {
                $TestOutput | Should -Match "Launching Docker services ...;Starting the NoLoadBalancingServices Docker Compose services;Starting the NoLoadBalancingServicesDockerComposeServicesOrchestrator process;Starting the NoLoadBalancingServices Docker Compose services logger;Starting the NoLoadBalancingServicesDockerComposeServicesLogger process;"
        ...
        BeforeEach {
            ...
            function Assert-ContainersAreStarted {
                ...
                if (-not($global:CleanupTriggered)) {
                    if (($MatchedRunningServicesCounter -eq 4) -and ($TotalRunningServicesCounter -eq $MatchedRunningServicesCounter)) {
                        $global:CleanupTriggered = $true
                        return $true
                    }

                    if ($MatchedRunningServicesCounter -ne 4) {
                        $global:CleanupTriggered = $true
