...
Describe 'Runner' {
    ...
    Context 'Environment context configuration' -Tag EnvironmentContextConfiguration -ForEach @(
        ...
        @{ Options = @("--source-only"); ExpectedOptions = @{
            Build = $true
            Start = $true
            SourceOnly = $true
            LoadBalancing = $true
        ...
        @{ Options = @("--source-only", "--no-build"); ExpectedOptions = @{
            Build = $false
            Start = $true
            SourceOnly = $true
            LoadBalancing = $true
        ...
        @{ Options = @("--source-only", "--no-build", "--no-start"); ExpectedOptions = @{
            Build = $false
            Start = $false
            SourceOnly = $true
            LoadBalancing = $true
        ...
        @{ Options = @("--source-only", "--no-build", "--no-start", "--no-load-balancing"); ExpectedOptions = @{
            Build = $false
            Start = $false
            SourceOnly = $true
            LoadBalancing = $false
...
'@
        ...
        It "configures the environment properties according to the options passed to the script : Build = <expectedoptions.build>; Start = <expectedoptions.start>; SourceOnly = <expectedoptions.sourceonly>; LoadBalancing = <expectedoptions.loadbalancing>; ExpectedRunTimes = <expectedruntimes>" {
            ...
            [MockedRunner]::EnvironmentContext.SourceOnly | Should -BeExactly $ExpectedOptions.SourceOnly
    ...
    Context 'Environment variables auto-configuration' -Tag EnvironmentVariablesConfiguration {
                ...
                Context 'General configuration' -ForEach @(
                    ...
                    BeforeEach {
                        ...
                        if ($LoadBalancing) {
                            ...
                            [Runner]::Main(@("--source-only", "--no-load-balancing"))
                        }
        ...
        Context 'Ignore configuration if not needed' {
            BeforeEach {
                [Runner]::Main(@("--source-only", "--no-start"))
            }
