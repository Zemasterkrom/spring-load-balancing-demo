...
configure_environment_variables() {
    if [ -n "${CONTAINER_NAME_ID}" ]; then
        ...
        mkdir -p "/usr/ldservice/config/${CONTAINER_NAME_ID}"
        touch "/usr/ldservice/config/${CONTAINER_NAME_ID}/system_hostname"
        load_and_persist_variables "${API_HOSTNAME}" "/usr/ldservice/config/${CONTAINER_NAME_ID}/system_hostname"
    fi
